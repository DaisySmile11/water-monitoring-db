...
exports.upsertAlertSettings = upsertAlertSettings;
exports.getAlerts = getAlerts;
exports.evaluateReading = evaluateReading;
...
async function getSettings(deviceId) {
    const r = await db_1.pool.query(`SELECT * FROM alert_settings WHERE device_id = $1`, [deviceId]);
...
async function upsertAlertSettings(req, res) {
    try {
        const deviceId = req.params.deviceId;
        const { salinity_high, ph_low, ph_high, temperature_low, temperature_high, battery_low, no_data_minutes, cooldown_minutes, email_to, enabled } = req.body || {};
        const result = await db_1.pool.query(`INSERT INTO alert_settings(
...
async function getAlerts(req, res) {
    try {
        ...
        if (device_id) {
            const r = await db_1.pool.query(`SELECT * FROM alerts WHERE device_id = $1 ORDER BY created_at DESC LIMIT $2`, [String(device_id), lim]);
            return res.json(r.rows);
        }
        const r = await db_1.pool.query(`SELECT * FROM alerts ORDER BY created_at DESC LIMIT $1`, [lim]);
        res.json(r.rows);
    }
    catch (err) {
        console.error(err);
...
async function shouldCooldown(deviceId, type, cooldownMinutes) {
    const r = await db_1.pool.query(`SELECT created_at FROM alerts WHERE device_id = $1 AND type = $2 ORDER BY created_at DESC LIMIT 1`, [deviceId, type]);
...
async function createAlert(params) {
    const r = await db_1.pool.query(`INSERT INTO alerts(device_id, type, severity, message, value, threshold, email_to)
...
async function evaluateReading(req, res) {
    try {
        const body = (req.body || {});
      ...
      `;
            const ok = await (0, email_1.sendEmail)({ to: emailTo, subject, html });
            if (ok) {
                await db_1.pool.query(`UPDATE alerts SET email_sent = TRUE WHERE id = ANY($1::bigint[])`, [created.map((a) => a.id)]);
            }
        }
